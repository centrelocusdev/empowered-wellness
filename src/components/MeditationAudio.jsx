      ...
      {data.map(({ title, desc, from_grad, audio }, key) => (
        ...
        >
          <div className="w-1/5 mb-3 md:mb-0">
            <button onClick={() => togglePlay(key)} className="text-2xl bg-light-skyblue text-gray-600 p-1 rounded-full">
              {currentAudioIndex === key && isPlaying ? <FiPauseCircle /> : <FiPlayCircle />}
            </button>
          </div>
