...
const Assessments = () => {
  ...
  const [epdsRes, setEpdsRes] = useState(() => {
    const storedResponses = localStorage.getItem('epdsResponses')
    return storedResponses ? JSON.parse(storedResponses) : []
  })
        ...
        {type === types[0] && (
          <div>
            ...
            {dassQuestions.length ? (
              <>
                {console.log(toggleModal, 'dassquestions')}
            ...
            <div
              className={` h-screen w-screen bg-[rgba(0,0,0,0.4)] fixed flex justify-center items-center`}
            >
              {console.log(toggleModal, 'inhidden')}
              <div className="bg-white shadow-lg rounded-3xl md:w-2/5  md:h-3/5 md:m-auto m-5 md:p-12 p-5  ">
                <div className=" mx-auto  ">
                  ...
                  <h2 className="text-lg">{`Your welleness measure is ${dassAvg}`}</h2>
                  <div className=" max-h-fit max-w-full  relative  bg-left-top">
                    <ButtonPrimary
                      text={'Mindfulness'}
            ...
            >
              <div className="bg-white shadow-lg rounded-3xl md:w-2/5  md:h-3/5 md:m-auto m-5 md:p-12 p-5  ">
                <div className=" mx-auto  ">
                  ...
                  <h2 className="text-lg">{`Your welleness measure is ${phqAvg}`}</h2>
                  <div className=" max-h-fit max-w-full  relative  bg-left-top">
                    <ButtonPrimary
                      text={'Mindfulness'}
                      handleClick={() => navigate('/mindfulness')}
                  ...
                  {q.options.map((option, i) => (
                    <div key={option.id} className="">
                      <input
                        ...
                        name={q.id}
                        value={option.id}
                        checked={epdsRes.find(
                          (r) => r.question === q.id && r.option === option.id
                        )}
                        onChange={() => handleEpdsRes(q.id, option.id)}
                        className="focus:accent-gray-900 accent-gray-800 mr-2"
                      />
                      <label>{option.text}</label>
                    </div>
